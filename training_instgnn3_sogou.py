...
from models.definitions.InstGNN_3 import InstGNN
from utils.data_loading import load_sogou_graph_data
from utils.constants import *
import utils.utils as utils
from utils.utils import get_main_loop
...
def train_gat_cora(config):
    ...
    # node_labels B N
    # edge_index B N * N
    _, train_node_features, train_node_labels, train_edge_features, train_edge_connection, train_edge_label = load_sogou_graph_data(
        config,
        "train",
        device)
    _, valid_node_features, valid_node_labels, valid_edge_features, valid_edge_connection, valid_edge_label = load_sogou_graph_data(
        config,
        "valid",
        device)

    assert len(train_node_features) == len(train_node_labels) == len(train_edge_features) == len(
        train_edge_connection) == len(train_edge_label)
    assert len(valid_node_features) == len(valid_node_labels) == len(valid_edge_features) == len(
        valid_edge_connection) == len(valid_edge_label)
    ...
    for epoch in range(config['num_of_epochs']):
        # Training loop
        train_graph_data = (
            train_node_features, train_node_labels, train_edge_features, train_edge_connection, train_edge_label)
        _, _, _, _, _, _ = main_loop(LoopPhase.TRAIN, train_graph_data, epoch=epoch)
        ...
        with torch.no_grad():
            try:
                ...
                valid_graph_data = (
                    valid_node_features, valid_node_labels, valid_edge_features, valid_edge_connection,
                    valid_edge_label)
                time_start = time.time()
                _, _, loss, node_accuracy, edge_p, edge_r = main_loop(LoopPhase.VAL, valid_graph_data, epoch=epoch)
                print("GAT validation: time elapsed=" + str(time.time() - time_start) + "[s]|epoch=" + str(
                    epoch + 1) + "|valid node acc=" + str(node_accuracy) + "|valid edge p=" + str(
                    edge_p) + "|valid edge r=" + str(edge_r))
                accuracy = node_accuracy + edge_p + edge_r
