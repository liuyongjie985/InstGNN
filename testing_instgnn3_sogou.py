...
def test_gat_cora(config):
    ...
    valid_file_name, valid_node_features, valid_node_labels, valid_edge_features, valid_edge_connection, valid_edge_label = load_sogou_graph_data(
        config, "valid", device)
    ...
    with torch.no_grad():
        try:
            print("test starting...")
            valid_graph_data = (
                valid_node_features, valid_node_labels, valid_edge_features, valid_edge_connection, valid_edge_label)
            time_start = time.time()
            node_class_pd_list, edge_class_pd_list, loss, node_accuracy, edge_p, edge_r = main_loop(LoopPhase.VAL,
                                                                                                    valid_graph_data,
                                                                                                    epoch=0)
            print("GAT test: time elapsed=" + str(time.time() - time_start) + "[s]|test node acc=" + str(
                node_accuracy) + "|test edge p=" + str(edge_p) + "|test edge r=" + str(edge_r))
            print("pic drawing")
