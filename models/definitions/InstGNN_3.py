...
class JointLearningLayer(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        in_nodes_features, in_edges_features, connectivity_mask = data  # unpack data
        ...
        # N * N * 21 => N * N * FOUT
        edges_features_proj = self.W_f1(dropout_edges_features)
        # N * N * FOUT
        m_ij = self.leakyReLU(edges_features_proj)
        # N * N * FOUT
        nodes_features_proj = self.W_f2(
            (node_source_feature + node_target_feature) * (node_source_feature + node_target_feature))
        # N * N * FOUT
        r_ij = self.leakyReLU(nodes_features_proj)

        out_edges_features = self.W_f3(torch.cat([m_ij, r_ij], dim=-1)) * self.softmax(connectivity_mask).unsqueeze(-1)
        # =====================================================================================================
        return out_nodes_features, out_edges_features, connectivity_mask
...
class InstGNNLayerImp2(InstGNNLayer):
    ...
    def forward(self, data):
        ...
        out_nodes_features = self.skip_concat_bias(all_attention_coefficients, in_nodes_features, out_nodes_features)
        return (out_nodes_features, in_edges_features, connectivity_mask)
