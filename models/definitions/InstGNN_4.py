...
class NoJointLearningLayer(torch.nn.Module):
    head_dim = 1

    # num_in_edge_features 始终不变
    def __init__(self, ell_edge_num_in_features, ell_sparse_edge_num_in_features, ell_edge_num_out_features,
                 ell_node_num_in_features,
        ...
        self.ell_edge_num_in_features = ell_sparse_edge_num_in_features
        ...
        self.W_f1 = nn.Linear(ell_sparse_edge_num_in_features, ell_edge_num_out_features)
        ...
        self.norm_node = nn.BatchNorm1d(nll_node_num_of_heads * nll_node_num_out_features, track_running_stats=False) if concat else nn.BatchNorm1d(nll_node_num_out_features,  track_running_stats=False)
...
class InstGNNLayer(torch.nn.Module):
    ...
    def __init__(self, num_in_features, num_in_edge_features, num_out_features, num_of_heads, layer_type, concat=True,
        ...
        self.log_attention_weights = log_attention_weights  # whether we should log the attention weights
        self.attention_weights = None  # for later visualization purposes, I cache the weights here
        self.norm = nn.BatchNorm1d(num_of_heads * num_out_features, track_running_stats=False) if concat else nn.BatchNorm1d(num_out_features, track_running_stats=False)
        self.init_params(layer_type)
