...
def normTotal(all_diagram_feature):
    node_dim = 27
    node_avg_vc = [0 for x in range(node_dim)]
    node_sd_vc = [0 for x in range(node_dim)]
    node_length = 0

    edge_dim = 21
    edge_avg_vc = [0 for x in range(edge_dim)]
    edge_sd_vc = [0 for x in range(edge_dim)]
    edge_length = 0

    for strokes_feature, strokes_label, edge_feature_matrix, edge_label, remove_id_map in all_diagram_feature:
        for fea_vc in strokes_feature:
            for i, item in enumerate(fea_vc):
                node_avg_vc[i] += item
            node_length += 1

        for _ in edge_feature_matrix:
            for fea_vc in _:
                if fea_vc == None:
                    continue
                for i, item in enumerate(fea_vc):
    ...
    for i, x in enumerate(node_avg_vc):
        node_avg_vc[i] /= node_length
    for i, x in enumerate(edge_avg_vc):
        edge_avg_vc[i] /= edge_length

    for strokes_feature, strokes_label, edge_feature_matrix, edge_label, remove_id_map in all_diagram_feature:
        for fea_vc in strokes_feature:
            for i, item in enumerate(fea_vc):
                node_sd_vc[i] += (item - node_avg_vc[i]) ** 2
        for _ in edge_feature_matrix:
            for fea_vc in _:
                if fea_vc == None:
                    continue
                for i, item in enumerate(fea_vc):
                    edge_sd_vc[i] += (item - edge_avg_vc[i]) ** 2

    for i, x in enumerate(node_sd_vc):
        node_sd_vc[i] /= node_length
        node_sd_vc[i] = node_sd_vc[i] ** 0.5
    for i, x in enumerate(edge_sd_vc):
        edge_sd_vc[i] /= edge_length
        edge_sd_vc[i] = edge_sd_vc[i] ** 0.5

    new_all_diagram_feature = []
    for strokes_feature, strokes_label, edge_feature_matrix, edge_label, remove_id_map in all_diagram_feature:
        temp_strokes_feature = nodeFeatureStandardization(strokes_feature, node_avg_vc, node_sd_vc)
        temp_edge_feature_matrix = edgeFeatureStandardization(edge_feature_matrix, edge_avg_vc, edge_sd_vc)
        new_all_diagram_feature.append(
            [temp_strokes_feature, strokes_label, temp_edge_feature_matrix, edge_label, remove_id_map])
    return new_all_diagram_feature
...
def nodeFeatureStandardization(node_feature, node_avg_vc, node_sd_vc):
    new_node_feature = []
    for fea_vc in node_feature:
        new_node_feature.append(vectorStandardization(fea_vc, node_avg_vc, node_sd_vc))
    return new_node_feature


def edgeFeatureStandardization(edge_feature, avg_vc, sd_vc):
    new_edge_feature = []
